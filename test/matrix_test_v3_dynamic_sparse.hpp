#ifndef MATRIX_TEST_DYNAMIC_SPARSE_H
#define MATRIX_TEST_DYNAMIC_SPARSE_H
...
namespace dynamic_sparse_matrix_test {
    ...
    auto testIntAssign() ->void {
        DMatrix_S<size_t> m(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        DMatrix_S<size_t> n(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        for(auto i = 0; i < m.rows(); i++){
            for(auto j = 0; j < m.cols(); j++){
                m.set_element(i + j,i,j);
            }
        }
        START
        n = m;
    ...
    auto testIntAdd() ->void {
        DMatrix_S<size_t> m(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        DMatrix_S<size_t> n(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        DMatrix_S<size_t> sum(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
    ...
    auto testIntSub() ->void {
        DMatrix_S<size_t> m(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        DMatrix_S<size_t> n(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        DMatrix_S<size_t> test(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
    ...
    auto testIntSubAssign() ->void {
        DMatrix_S<size_t> m(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        DMatrix_S<size_t> n(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        DMatrix_S<size_t> diff(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
    ...
    auto testIntMul() ->void {
        ...
        for(auto i = 0; i < m.rows(); i++){
            for(auto j = 0; j < m.cols(); j++){
                m.set_element((i*m.cols() + j + 1),i,j);;
            }
        }
        for(auto i = 0; i < m.rows(); i++){
            for(auto j = 0; j < m.cols(); j++){
                n.set_element((i*m.cols() + j + 1),i,j);;
            }
        }
    ...
    auto testIntMulAssign() ->void {
        ...
        for(auto i = 0; i < n.rows(); i++){
            for(auto j = 0; j < n.cols(); j++){
                n.set_element((i*n.cols() + j + 1),i,j);
            }
        }
    ...
    auto testComplexAssign() ->void {
        DMatrix_S<complex<size_t>> m(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        DMatrix_S<complex<size_t>> n(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        for(auto i = 0; i < m.rows(); i++){
            for(auto j = 0; j < m.cols(); j++){
                m.set_element(complex(i + j,i + j),i,j);;
            }
        }
    ...
    auto testComplexCompare() ->void {
        ...
        for(auto i = 0; i < m.rows(); i++){
            for(auto j = 0; j < m.cols(); j++){
                n.set_element(complex(i + j,i + j),i,j);;
            }
        }
        ...
        n.set_element(complex(1324,0),0,0);
    ...
    auto testComplexAdd() ->void {
        DMatrix_S<complex<size_t>> m(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        DMatrix_S<complex<size_t>> n(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        DMatrix_S<complex<size_t>> sum(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        ...
        for(auto i = 0; i < m.rows(); i++)
            for(auto j = 0; j < m.cols(); j++)
                sum.set_element(complex(2 * (i + j),2 * (i + j)),i,j);
    ...
    auto testComplexSub() ->void {
        DMatrix_S<complex<size_t>> m(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        DMatrix_S<complex<size_t>> n(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        DMatrix_S<complex<size_t>> test(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        for(auto i = 0; i < m.rows(); i++){
            for(auto j = 0; j < m.cols(); j++){
                m.set_element(complex(i + j,i + j),i,j);;
            }
    ...
    auto testComplexSubAssign() ->void {
        DMatrix_S<complex<size_t>> m(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        DMatrix_S<complex<size_t>> n(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        DMatrix_S<complex<size_t>> diff(MATRIX_TEST_SIZE, MATRIX_TEST_SIZE);
        for(auto i = 0; i < m.rows(); i++){
            for(auto j = 0; j < m.cols(); j++){
                m.set_element(complex(i + j,i + j),i,j);;
            }
    ...
    auto testComplexMul() ->void {
        ...
        for(auto i = 0; i < m.rows(); i++){
            for(auto j = 0; j < m.cols(); j++){
                m.set_element(complex(i * m.cols() + j + 1,i * m.cols() + j + 1),i,j);;
            }
        }
        for(auto i = 0; i < m.rows(); i++){
            for(auto j = 0; j < m.cols(); j++){
                n.set_element(complex(i * m.cols() + j + 1,i * m.cols() + j + 1),i,j);;
            }
        }
...
#endif // MATRIX_TEST_DYNAMIC_SPARSE_H
